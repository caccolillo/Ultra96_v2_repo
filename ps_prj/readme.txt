...
architecture RTL of radio_link_1wire is
    ...
    -- -------------------------------------------------------------------------
    --! @brief New-link bootstrap state written by ARM SW at startup.
    --!        When both units start with RS=LINK_STATE_NEW, the self-echo
    --!        rejection would normally block all frames because both sides
    --!        transmit identical payloads.  Frames carrying LINK_STATE_NEW
    --!        are always accepted regardless of the local RS value, allowing
    --!        the SW timer mechanism to progress to Active/Inactive negotiation.
    -- -------------------------------------------------------------------------
    constant LINK_STATE_NEW : std_logic_vector(DATA_BITS - 1 downto 0) := x"AA";
        ...
        if rising_edge(CLK_I) then
            ...
            if rst = '1' or PrngReady = '0' then
                ...
                case LinkState is
                    ...
                    --!   1. RxBuf(0) = SYNC_BYTE  (valid frame / correct alignment)
                    --!   2. Payload differs from local state  OR  RS = LINK_STATE_NEW
                    --!
                    --! Condition 2 has two sub-cases:
                    --!   a. Normal operation: payload differs from local state,
                    --!      meaning the frame genuinely came from the remote unit
                    --!      rather than being an open-drain loopback of our own TX.
                    --!   b. Bootstrap exception: if RxBuf(1) = LINK_STATE_NEW (0xAA),
                    --!      always accept regardless of local RS.  At startup both
                    --!      units write RS=0xAA (LINK_STATE_NEW), so their frames
                    --!      are identical and would be rejected as self-echo under
                    --!      the normal condition.  Accepting NEW_LINK frames allows
                    --!      PRS to be updated to 0xAA, which the ARM SW detects as
                    --!      a new link and uses to start the Active/Inactive timer.
                    --!      Once one unit calls GoActive() and writes RS=0x41, its
                    --!      frames are no longer NEW_LINK and normal rejection resumes.
                    --!
                    --! On self-echo (payload matches local state, not NEW_LINK): do NOT
                    --!      go to Waiting.  Let ByteCount run to timeout so MissedFrames
