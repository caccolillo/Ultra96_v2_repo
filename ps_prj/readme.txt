...
module tb_axi4lite_to_wishbone_bridge_integrity;
  ...
  initial begin
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++) begin
      ...
      // --- Check 1: write REG_FPGA_ALIVE_DSP (offset 3) then read back ---
      // This is a real r/w register in syscon. Avoid values that match
      // reset state (0x0000) to make mismatches unambiguous.
      wr_val = $urandom_range(1, 32'hFFFF);

      axi_write(32'h0000_0003, wr_val, timeout_flag);
      if (timeout_flag) begin
        hangs++;
        $display("[%0d] WRITE HANG", i);
      end else begin
        axi_read(32'h0000_0003, rd_val, timeout_flag);
        if (timeout_flag) begin
          hangs++;
