...
module tb_radio_link_1wire_pat6570;
    ...
    //   Sim uses DEFAULT_DIVISOR=5 (VHDL default overridden via generic in sim)
    ...
    // At divisor=5 this is 481 * BYTE_PERIOD_NS
    localparam real RECEIVE_TIMEOUT_NS = 481.0 * BYTE_PERIOD_NS;

    // Link-down timeout from hardware logs: ~3s on real HW, scaled to sim
    // LINK_WAIT_PERIOD=46 byte-times at divisor=5
    ...
    // wb_write_a: drive address and data one full cycle BEFORE asserting STB
    // so data is stable at the posedge the RTL samples WB_DAT_I.
    // The RTL asserts WB_ACK_O for exactly one cycle on the same posedge it
    // samples the write data, so we must not sample ACK until data is stable.
    task automatic wb_write_a (input logic [2:0]        reg_off,
                               input logic [WB_DAT_W-1:0] data);
        while (rst_a) @(posedge clk);
        // Step 1: drive address and data at negedge — one full cycle early
        @(negedge clk);
        a_adr  = reg_off;
        a_wdat = data;
        a_we   = 1'b1;
        // Step 2: let data settle through one full clock before asserting STB
        @(posedge clk);
        // Step 3: assert STB/CYC at the following negedge
        @(negedge clk);
        a_stb  = 1'b1;
        a_cyc  = 1'b1;
        // Step 4: wait for ACK — RTL asserts on the posedge it samples data
        do @(posedge clk); while (!a_ack);
        // Step 5: deassert at negedge after ACK seen
        @(negedge clk);
        a_stb  = 1'b0;
        a_cyc  = 1'b0;
        a_we   = 1'b0;
        // Step 6: hold data/adr stable one more cycle THEN clear
        @(posedge clk);
        a_adr  = 3'h0;
        a_wdat = 16'h0000;
    ...
    task automatic check_invariants (input string ctx, input int chk_iter);
        ...
        if (chk_rs_a[7:0] == ST_NO_LINK && chk_rs_b[7:0] == ST_NO_LINK) begin
            ...
            // [5] Self-echo: A.PRS != A.RB
            // Only check when both values are non-zero (0x00 = no-link,
            // not a self-echo condition)
    ...
    initial begin : test_main
        ...
        // Assert reset immediately at time 0 so divisor reg and all
        ...
        $display("  Clock : 80 MHz   Baud : 9600   DIVISOR : 5 (sim)  HW: 520");
        ...
        //         NUM_REBOOT_ITER=5 here for rapid reproduction.
        //         DUT A or DUT B reboots while the peer is mid-cycle.
        //         With the buggy RTL this reproduces PAT6-570 within ~10 iter.
        //         Matches the hardware failure window from log analysis:
        ...
        for (iter = 0; iter < NUM_REBOOT_ITER; iter++) begin
            ...
            // Full settle between iterations so previous link cycle completes
            #(SETTLE_NS);
        end
        ...
        //         After successful negotiation (A=Active, B=Inactive),
        //         manually writing B to Active should trigger A to go Inactive.
        //         Bug: toggled radio reverts; changeover only works on power-off.
        //         Uses module-level signals t6_prs_a, t6_prs_b, t6_rs_a, t6_rs_b
        // -----------------------------------------------------------------
        ...
        // Verify baseline negotiation is correct before toggle
        ...
        // Now manually toggle B from Inactive to Active
        $display("  Toggling B: Inactive -> Active");
        set_states_b(ST_ACTIVE, BT_FULL_SVC);

        // Wait for re-negotiation — peer should detect conflict and yield
        #(SETTLE_NS);
        ...
        // After toggle, exactly one radio should be Active and one Inactive
        // and PRS registers should reflect each other correctly
        ...
        //         Active radio (A) loses link — Inactive peer (B) should
        //         go Active within RECEIVE_TIMEOUT.
        //         Simulates power-off of A by holding rst_a high.
        //         From hardware logs: changeover DOES work on power-off.
        //         Uses module-level signal t7_prs_b
        // -----------------------------------------------------------------
        ...
        // Hold A in reset (simulates power-off / link gone)
        $display("  Holding A in reset (link-down)");
        @(negedge clk);
        rst_a = 1'b1;

        // Wait for B to detect link broken and go Active
        // RECEIVE_TIMEOUT = 481 byte-times at sim divisor
        #(RECEIVE_TIMEOUT_NS * 2.0);

        wb_read_b(REG_PRS, t7_prs_b);

        total_checks++;
        $display("  B.PRS(502)=%02Xh after link-down (expect %02Xh=no-link or %02Xh=active)",
                 t7_prs_b[7:0], ST_NO_LINK, ST_ACTIVE);
        // After A disappears, B's PRS should go to 0x00 (LinkBroken)
        // and B's ARM logic should then force it Active
        // We check that B is no longer seeing A's state (i.e. PRS cleared)
        if (t7_prs_b[7:0] === ST_NO_LINK || t7_prs_b[7:0] === 8'h00) begin
        ...
        // Release A from reset
        @(negedge clk);
        rst_a = 1'b0;
        repeat (4) @(posedge clk);

        // -----------------------------------------------------------------
        // TEST 8  Reboot race — matches hardware failure window exactly
        //         From messages_237006.txt / messages_216882.txt (11-Sep-2026):
        //           T6-237006 (A): boots, LINK Down at T+3s, forces Active
        //           T6-216882 (B): running, loses link, timeout at T+6s
        //         The 3s window in HW = LINK_WAIT_PERIOD=46 byte-times in RTL
        //         The 6s window in HW = RECEIVE_TIMEOUT=481 byte-times in RTL
        //         With the fixed RTL, B must receive a valid frame from A
        //         within the RECEIVE_TIMEOUT window after A reboots.
        //         Uses module-level signal t8_prs_b
        // -----------------------------------------------------------------
        ...
        // Simulate A rebooting mid-cycle while B is running
        $display("  A rebooting mid-cycle (B running, peer link active)");
        reset_a_only();
        set_states_a(ST_ACTIVE, BT_FULL_SVC);

        // Wait exactly RECEIVE_TIMEOUT — this is the critical window.
        // With the fixed RTL, A should have transmitted a valid frame
        // and B should have committed it before this timeout fires.
        #(RECEIVE_TIMEOUT_NS);
    ...
    initial begin : watchdog
        #(64'd10_000_000_000);  // 10 sec sim-time ceiling
        $display("WATCHDOG TIMEOUT at %0t us -- increase timeout or reduce NUM_REBOOT_ITER",
                 $time/1000);
        $finish;
    end : watchdog
