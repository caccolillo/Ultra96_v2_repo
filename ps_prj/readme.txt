...
printf 'if {![file exists $bit_file]} { error "bit file not found: $bit_file" }\n' >> "$TCL_GEN"
...
# Commands taken verbatim from Vivado GUI journal after successful flash programming
printf 'set bit_file  "%s"\n' "$BIT_FILE"   > "$TCL_PROG"
printf 'set mcs_file  "%s"\n' "$MCS_FILE"  >> "$TCL_PROG"
...
# Set programming properties on cfgmem (already associated from previous session
# or will be created). Matches exactly what Vivado GUI does.
set_property PROGRAM.ADDRESS_RANGE  {use_file} [get_property PROGRAM.HW_CFGMEM $dev]
set_property PROGRAM.FILES          [list $mcs_file] [get_property PROGRAM.HW_CFGMEM $dev]
set_property PROGRAM.PRM_FILE       $prm_file [get_property PROGRAM.HW_CFGMEM $dev]
...
startgroup
# create_hw_bitstream loads the indirect programming proxy into the FPGA
# so it can act as SPI bridge -- this is what the GUI does before program_hw_cfgmem
